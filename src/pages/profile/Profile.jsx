...
const Profile = () => {
  ...
  const { userPostsCount, userFilteredPosts } = useSelector(
    (state) => state.profile
  );
          ...
          <section className="box-border w-auto rounded-xl 2xl:gap-2 xl:gap-2 lg:gap-2 grid 2xl:grid-flow-col xl:grid-flow-col lg:grid-flow-col  grid-cols-12 border-[1px] min-h-max mb-10  p-12  ">
            <div className=" box-border w-auto flex 2xl:col-span-2 xl:col-span-3 lg:col-span-4 md:col-span-12 sm:col-span-12 flex-row  xl:justify-start lg:justify-start md:justify-center sm:justify-center items-center xl:h-[190px] lg:h-[190px]">
              <div className=" relative">
                <Avatar
                  src={
                    img ? URL.createObjectURL(img) : profile?.profilePhoto?.url
                  }
                  alt=""
                ...
                {user?._id === profile?._id && (
                  <div
                    className="absolute z-0 top-[40%] left-[100%]"
                    ...
                    >
                        ...
                        return (
                          <Form title="upload new photo">
                            <div>
                              <label
                                htmlFor="image"
                                className="text-center  cursor-pointer inline-block relative z-10 left-[-11px] px-[7px] pt-[5px] pb-[2.5px] rounded-full bg-[#ffffff] "
                              >
                                {formik.isSubmitting ? (
                      ...
                      {msg}
                            ...
                            <button
                              ...
                              type="submit "
                              className="relative text-faint-gray text-sm font-normal"
                            >
                              upload
                            </button>
                          </Form>
            ...
            <div className="md:mt-2 sm:mt-2  min-w-min flex 2xl:col-span-10 xl:col-span-9 lg:col-span-8 md:col-span-12 sm:col-span-12 flex-col xl:items-start lg:items-start md:items-center sm:items-center justify-between box-border ">
              <div className="flex flex-col w-full xl:items-start lg:items-start md:items-center sm:items-center xl:justify-end lg:justify-end box-border">
                <span className="text-blue-black font-sm " title="author">
                  Author
                </span>
              ...
              <div className="  flex xl:flex-row lg:flex-row md:flex-col sm:flex-col items-center justify-between w-full h-auto ">
                  ...
                  {profile?.socialMedia?.map((social, index) => {
                    if (social?.name === "github") {
                      return (
                        <Link
                          key={index}
                          data-title="follow me on github"
                          className="smIcon0"
                          target="_blank"
                          to={`${social?.url}`}
                          rel="noopener noreferrer"
                    ...
                    if (social?.name === "gmail") {
                      return (
                        <Link
                          ...
                          className="smIcon0"
                          target="_blank"
                          to={`https://mail.google.com/mail/u/0/?view=cm&fs=1&to=${social.url}`}
                          data-title="follow me on gmail"
                        >
                    ...
                    if (social?.name === "whatsapp") {
                      return (
                        <Link
                          ...
                          className="smIcon0"
                          target="_blank"
                          to={`https://wa.me/${social?.url}?text=Hi,%20I'd%20like%20to%20chat%20with%20you%20on%20WhatsApp`}
                          data-title="follow me on whatsapp"
                        >
                          <i className="fa-brands fa-whatsapp lg:text-xl md:text-base sm:text-base"></i>
                        </Link>
                      );
                    }
                  })}
                ...
                <div className="flex flex-row justify-between items-center space-x-2 md:mt-4 sm:mt-4 ">
                  <div className="self-center text-center  box-border flex flex-row items-center justify-between space-x-1 rounded-full bg-[#3234400a] w-auto px-5 py-2 xl:mt-0 lg:mt-0  ">
                    ...
                    <span className="text-xs block">Articles</span>
                  </div>

                  {user._id === profile?._id && (
                    <a
                      className={hoverEff ? "smIcon00" : ""}
          ...
          <div className="box-border  grid   xl:gap-y-7 lg:gap-y-7 md:gap-y-2 sm:gap-y-2 ">
            ...
            {userFilteredPosts.length > 0 ? (
              userFilteredPosts?.map((post, index) => (
                <article
                  key={index}
                  className="box-border w-full   bg-white h-auto p-5 grid grid-cols-12 rounded-2xl relative border-[1px] border-[#8589a038]  transition-all duration-[842.435ms] ease-in-out "
                >
                  <div className=" flex flex-row items-center justify-start xl:p-5 lg:p-5  box-border h-full w-full  xl:col-span-4 lg:col-span-4 md:col-span-12  sm:col-span-12  md:mb-3 sm:mb-3">
                    <Link
                      className="box-border  w-full block h-full image rounded-2xl overflow-hidden  "
                    ...
                    >
                      <div className="h-auto w-auto relative overflow-hidden  rounded-2xl">
                        <img
                          src={post?.image?.url}
                          alt={post?.title}
                          className=" inline-block cursor-pointer rounded-2xl w-full h-[301px]    object-cover transition-all duration-[.7s] ease-in-out hover:scale-[1.2]"
                        />

                        <Link to={`/posts/categories/${post?.category}`}>
                          <span className=" inline-block w-auto  bottom-[30px] left-0  absolute bg-black text-white px-4 py-[7px] text-xs whitespace-normal">
                            {" "}
                            {post?.category}
                          </span>
                        </Link>
                  ...
                  <div className="box-border flex flex-col h-auto xl:justify-center lg:justify-center items-start md:space-y-5 sm:space-y-5  xl:col-span-8 lg:col-span-8 md:col-span-12 sm:col-span-12 m:mb-4 sm:mb-4 xl:p-5 lg:p-5 ">
                    <h3 className="title min-h-max  w-full ">
                      <a
                        className=" titleLink  cursor-pointer w-full break-normal "
                    ...
                    <div className="info flex flex-row items-center italic justify-between flex-nowrap min-h-[48px] ">
                      <div className="author flex flex-row items-center justify-between">
                        ...
                        <div>
                          <span className="mx-1 text-[13px] whitespace-nowrap ">
                            By
                          </span>
                          <Link className="font-bold text-[13px] whitespace-nowrap">
                            {profile?.username}
                          </Link>
                        </div>
                      </div>
                    ...
                    <p className="text-sm leading-normal w-full overflow-hidden  h-[96px] ">
                      <span className="inline-block break-all h-full text-faint-gray">
                        {post?.description}
                      </span>
                    </p>

                    <Link
                      className="hover:text-white min-h-[41.6px] box-border  hover:bg-[#4776E6] transition-all duration-500 ease-in-out  border-[1px] rounded-full py-2 px-7 mr-2"
                      to={`/posts/details/${post?._id}`}
                    >
              ...
              <div className="box-border w-full h-auto my-10 flex flex-col items-center justify-start ">
                ...
                <p className="w-full  h-[10vh] text-blue-black font-semibold 2xl:3xl xl:text-3xl lg:text-2xl md:text-xl sm:text-base leading-tight text-center ">
                  No Posts Were Found
                </p>
              </div>
            )}
